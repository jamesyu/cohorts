...
Cohorts = (function () {
    ...
    var dataLayerAdapter = {
        ...
        onInitialize: function(inTest, testName, cohort, varSlot, scope, newToTest) {
        ...
        constructor.prototype = {
            ...
            run: function () {
                ...
                if (in_test || preview) {
                    ...
                    this.options.storageAdapter.onInitialize(in_test, this.options.name, chosen_cohort, this.options.varSlot, this.options.scope, newToThisTest);
            ...
            isExecutable:function()
            {
                ...
                var uaBlock = this.options.uaBlock?userAgentUtil.match(this.options.uaBlock):false;
                if (uaBlock)
                {
             ...
             * if all cohort objects has 'sample' property then use percentage based random
             * @returns cohort key
             */
            chooseCohort:function()
            {
                ...
                else
                {
                    ...
                    var weight = 0;
                    var lastSample = orderedCohorts[orderedCohorts.length-1].sample;
                    ...
                    while(!chosen_cohort)
                    {
                        ...
                        {
                            if (weight <= orderedCohorts[i].sample)
                            {
            ...
            getOrderedBySampleCohorts:function()
            {
                ...
                for (var p in cohorts)
                {
                    orderedCohorts.push({key: p, sample: cohorts[p].sample});
                }
    ...
    var Cookies = {
         ...
         * @return void
         */
        set: function(name, value, props, scope)
        {
            props = props || {};
