// Declare GA queue before we add events into it
var _gaq = _gaq || [];
...
Cohorts = (function () {
    ...
    var GoogleAnalyticsAdapter = {
        nameSpace: 'cohorts',
		setCustomVar: function(varSlot, action, opt_label, scope) {
			_gaq.push(['_setCustomVar', varSlot, action, opt_label, 2]);
		},
        trackEvent: function(category, action, opt_label, opt_value, int_hit) { 	
        ...
        onInitialize: function(inTest, testName, cohort, varSlot, scope) {
            if (inTest && varSlot) this.setCustomVar(varSlot, testName, cohort, scope)
        },
    ...
    var Test = (function () {
        ...
        constructor.prototype = {
            run: function () {
                ...
                if (in_test === null  && this.isExecutable()) // haven't seen this user before
                ...
                if (in_test || preview) {
                    ...
                    // process chosen Cohort directTo(direct to other page) if directTo property is available.
                    ...
                    // runNow: 0--execute onChosen Immediately, 1--execute onChosen when dom is ready, default is 0
             ...
             * this method will check 4 options, urlMatch, uaMatch, cookieMatch, referrerMatch
            ...
            isExecutable:function()
            {
                ...
                var uaMatch = userAgentUtil.match(this.options.uaMatch);
                if (!uaMatch)
                {
                    Utils.log("Test Object ["+this.options.name+"] user agent not matched.");
                    return false;
                }
             ...
             * urlMatch: [/test1.html/i, /test1.html/i], that means, the test object will run in test1.html or test2.html
             * or urlMatch:[
             * {
             * regex: /gclid=/i,
             * regexType:1 (0--regex will apply for document.location.href, 1--regex will apply for document.location.search)
             * }
            ...
            urlMatch: function()
            {
                ...
                for (var i= 0,c=urlRegexes.length;i<c;i++)
                {
                    ...
                    {
                        urlMatch = reg.test(document.location.search);
                    }
            ...
            /**
             * check if current page cookies value matched the test object cookieMatch option
             * cookieMatch is an array of cookie items, you can specify it like this:
             ...
             * or cookieMatch: [{c1: "c1value", c2: "c2value"}, {c3: "c3value"}]
             * that means, the test object will run when current (cookies.c1="c1value" and cookies.c2="c2value") or cookies.c3="c3value"
             ...
             * you can make some complex statement in this function, then return true means cookies matched, false means not matched. e.g.
            ...
            /**
             * check if current page referrer value matched the test object referrerMatch option
    ...
    var userAgentUtil = {
        ...
        /**
         * test if current browser's user agent matched the test object uaMatch options
         * @param tua test object uaMatch option, it's an array of ua items,
         * you can specify it like this:
         * uaMatch:["ie", "chrome"], that means test object will run on ie and chrome
         * uaMatch:[{ie:[9, 10]}, {chrome:[25, 26]}] that means test object will run on ie9, ie10, chrome25, chrome26
